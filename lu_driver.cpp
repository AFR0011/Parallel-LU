...
static int run_serial_baseline(const RunOptions& opt, int n, const MatrixSpec& ms) {
    ...
    // Generate full A and b on the driver (same generator used by workers)
    ...
    // In-place LU factorization with partial pivoting (same math as worker elimination)
    for (int k = 0; k < n; ++k) {
        // Pivot scan: find best row in [k..n-1] maximizing |A[i,k]|
        ...
        // Swap rows if needed (swap A and b)
        t0 = omp_get_wtime();
        if (best_gi != k) {
            ++swap_total;
            for (int j = 0; j < n; ++j) {
